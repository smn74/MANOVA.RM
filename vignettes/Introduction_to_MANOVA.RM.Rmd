...
* In a repeated measures designs (RM) the same outcome is observed at different occasions, e.g., different time points or different parts of the body like left/right hemisphere of the brain. Hypotheses about the sub-plot or within subjects factors are possible.
...
The `RM` function calculates the above mentioned test statistics in a repeated measures design with an arbitrary number of crossed whole-plot (between subjects) and 
