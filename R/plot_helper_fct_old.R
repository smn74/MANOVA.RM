...
helper_old <- function(plot.object, descr.object, factor, ...){
  ...
  if(nf == 1){
    ...
    if(factor %in% fac_names_twofold && nf == 2){
      ...
      h$y <- matrix(descr.object[, "Means"], ncol = length(levels[[posi2]]), 
                    byrow = T)
      nlower <- matrix(descr.object[, lo], ncol = length(levels[[posi2]]), 
                     byrow = T)
      rownames(nlower) <- levels[[posi]]
      colnames(nlower) <- levels[[posi2]]
      
      nupper <- matrix(descr.object[, up], ncol = length(levels[[posi2]]), 
                      byrow = T)
